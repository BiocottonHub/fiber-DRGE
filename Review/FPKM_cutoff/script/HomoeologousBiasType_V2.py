...
LastEditors: zpliu
LastEditTime: 2023-05-16 21:06:28
@param: 
'''
import pandas as pd 
...
# )
Fold_change=float(sys.argv[2])
accessionPercentage=float(sys.argv[3])

ExpressedGeneList=pd.read_csv(
    sys.argv[4],header=None,index_col=None,sep="\t"
)
...
def BiasSampleCount(homoeologousExpression,condition,fold_change):
    BiasACount=0
    BiasDCount=0
    cutoff=(fold_change-1)/(1+fold_change)
    for At,Dt in homoeologousExpression:
        if At>=condition and Dt>=condition:
            if (At-Dt)/(At+Dt) >=cutoff:
                BiasACount+=1 
            elif (At-Dt)/(At+Dt) <=-1*cutoff:
                BiasDCount+=1 
...
def homoeologousExpressionNot(genePair,expressedGeneList,conditions,totalSampleCount):
    ...
    if At not in ExpressedGeneList[0].values and Dt not in ExpressedGeneList[0].values:
        ...
        BiasACount,BiasDCount=BiasSampleCount(zip(At_Expression,Dt_Expression),0.5,Fold_change)
        if BiasACount/totalSampleCount>=accessionPercentage and BiasDCount/totalSampleCount<accessionPercentage:
            return ("BiasA",pavl,"BiasA") 
        elif BiasACount/totalSampleCount<accessionPercentage and BiasDCount/totalSampleCount>=accessionPercentage:
...
for genePair in Homoeologous.values:
    BiasTypeV1,pval,BiasType=homoeologousExpressionNot(genePair,ExpressedGeneList,0.1,expressionData.shape[1])
    out.append(
...
mergeData.to_csv(sys.argv[5],header=True,index=False,sep="\t")
